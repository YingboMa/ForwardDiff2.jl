...
@testset "Derivative, Gradient, Jacobian, and Hessian" begin
    ...
    # Hessian
    @test D(x -> D(x->x[1]^x[2] + x[3]^3 + x[3]*x[2]*x[1], x) * I, @SVector[1,2,3]) * I === @SMatrix [2 4 2; 4 0 1; 2 1 18.]
    @test D(x -> D(x->x[1]^x[2] + x[3]^3 + x[3]*x[2]*x[1], x) * I, [1,2,3]) * I == [2 4 2; 4 0 1; 2 1 18.]
end
...
@testset "Inference" begin
    ...
    @test_broken @inferred complicated_D * 1
end
